...
def get_tweets_sentiment(query, count=2500, start=None, end=None, lang='english'):
    '''
...
def get_tweets(query, count=2500, start=None, end=None, lang='english'):
    '''
...
if __name__ == '__main__':
    for item in country_list:
        query = 'sustainable development geocode:"{}"'.format(item[0])
        count = 2500
        with open('{}.csv'.format(item[1]), 'a') as csvfile:
            for tweet in get_tweets_sentiment(query, count):
                writer = csv.writer(csvfile)
                writer.writerow((tweet['sentiment'], tweet['text']))